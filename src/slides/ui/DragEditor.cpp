...
std::vector<slope::PrimitivePtr> slope::DragEditor::getPrimitivesUnderMouse(Slide& s, scalar x, scalar y) const {
    ...
    for (auto it = sorted.rbegin(); it != sorted.rend(); ++it) { // front to back
        ...
        if (std::abs(p(0) + off(0) / S.x - x) < prim_size(0) / 2 / S.x && std::abs(p(1) + off(1) / S.y - y) < prim_size(1) / 2 / S.y)
            hits.push_back(ptr);
    }
    return hits;
}
...
slope::PrimitivePtr slope::DragEditor::pickWithCycling(Slide& s, scalar x, scalar y) {
    ...
    bool same_spot = std::abs(x - last_pick_x) < same_spot_thr && std::abs(y - last_pick_y) < same_spot_thr && !pick_stack.empty();
...
void slope::DragEditor::handleMarquee(Slide& cs, WindowManager& wm) {
    ...
    if (!marquee_active && (std::abs(x - marquee_start_x) > marquee_thr || std::abs(y - marquee_start_y) > marquee_thr))
        marquee_active = true;
    ...
    if (!marquee_active) { // never travelled, so behave like a toggle-click
        ...
        for (auto& [pptr, sis] : cs.getScreenPrimitives()) {
            ...
            if (p(0) - hw < xmin || p(0) + hw > xmax || p(1) - hh < ymin || p(1) + hh > ymax)
                continue;
...
void slope::DragEditor::handle(Slide& cs, WindowManager& wm) {
    ...
    for (auto& [pptr, sis] : screen) {
        ...
        for (int di = 0; di < 3; ++di)
            for (int oi = 0; oi < 3; ++oi) {
                ...
                if (std::abs(dx) < std::abs(snap_dx)) {
                    snap_dx = dx;
                    guide_x = o_ax[oi];
                }
                float dy = o_ay[oi] - d_ay[di];
                if (std::abs(dy) < std::abs(snap_dy)) {
                    snap_dy = dy;
                    guide_y = o_ay[oi];
                }
            }
    ...
    if (!pending_undo.empty()) {
        ...
        if (std::abs(cx - pending_undo.front().state.x) > move_eps || std::abs(cy - pending_undo.front().state.y) > move_eps)
            commitUndo();
    }

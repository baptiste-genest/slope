...
void DrawTexturedPlane(const Frame3D& f, const ImageData& data, const RGBA& tint, scalar alpha) {
    ...
    for (int j = 0; j < N; j++) {
        for (int i = 0; i < N; i++) {
            ...
            dl->PrimWriteIdx((ImDrawIdx)a0);
            dl->PrimWriteIdx((ImDrawIdx)a1);
            dl->PrimWriteIdx((ImDrawIdx)a3);
            dl->PrimWriteIdx((ImDrawIdx)a0);
            dl->PrimWriteIdx((ImDrawIdx)a3);
            dl->PrimWriteIdx((ImDrawIdx)a2);
        }
    }
    dl->PopTexture();
}
...
std::optional<Transform> EvalLivePlane(const LivePlane& l) {
    ...
    if (u.norm() > 1e-9 && n.norm() > 1e-9 && u.normalized().cross(n.normalized()).norm() > 1e-6) {

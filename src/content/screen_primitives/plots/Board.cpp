...
RGBA nextInk(const std::string& board) {
    static const std::vector<RGBA> palette = {
        RGBA(0.35f, 0.62f, 0.98f, 1.f),
        RGBA(0.98f, 0.55f, 0.30f, 1.f),
        RGBA(0.38f, 0.80f, 0.52f, 1.f),
        RGBA(0.88f, 0.45f, 0.72f, 1.f),
        RGBA(0.95f, 0.82f, 0.32f, 1.f),
        RGBA(0.60f, 0.55f, 0.92f, 1.f),
    };
...
vec2 Board::step() const {
    ...
    return vec2(xlog() ? 1 : asked_x > 0 ? asked_x
                                         : niceStep((x(1) - x(0)) / 7),
                ylog() ? 1 : asked_y > 0 ? asked_y
                                         : niceStep((y(1) - y(0)) / 5));
}

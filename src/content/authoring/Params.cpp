...
void Params::ensureLoaded() {
    ...
    try {
        ...
        const std::string msg = "cannot read " + file().string() + " : " + e.what() + ", parameters keep their defaults and are not saved until it is fixed";
        spdlog::error("{}", msg);
...
int Params::read(const std::string& name, scalar* out4) {
    ...
    if (auto p = std::dynamic_pointer_cast<ColorEntry>(e)) {
        out4[0] = p->value.Value.x;
        out4[1] = p->value.Value.y;
        out4[2] = p->value.Value.z;
        out4[3] = p->value.Value.w;
        return 4;
    }
    if (auto p = std::dynamic_pointer_cast<ScalarEntry>(e)) {
        out4[0] = p->value;
        return 1;
    }
    if (auto p = std::dynamic_pointer_cast<IntEntry>(e)) {
        out4[0] = p->value;
        return 1;
    }
    if (auto p = std::dynamic_pointer_cast<BoolEntry>(e)) {
        out4[0] = p->value ? 1 : 0;
        return 1;
    }
    if (auto p = std::dynamic_pointer_cast<EnumEntry>(e)) {
        out4[0] = p->value;
        return 1;
    }
    return 0;
...
int Params::write(const std::string& name, const scalar* in, int n) {
    ...
    if (auto p = std::dynamic_pointer_cast<Vec2Entry>(e)) {
        p->value = vec2(in[0], in[1]);
        return written();
    }
    if (auto p = std::dynamic_pointer_cast<VecEntry>(e)) {
        p->value = vec(in[0], in[1], in[2]);
        return written();
    }
    if (auto p = std::dynamic_pointer_cast<DirEntry>(e)) {
        p->value = vec(in[0], in[1], in[2]);
...
Params::EnumParam Params::AddEnum(const std::string& name,
    ...
    if (index(def) < 0 && !e->options.empty())
        throw std::runtime_error("parameter \"" + name + "\" defaults to \"" + def + "\", which is not one of its options");
    if (!keepEditedValue(name)) {
...
bool Params::ColorEntry::drawUI(const char* label) {
    ...
    return ImGui::ColorEdit4(label, (float*)&value.Value,
                             ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_PickerHueWheel);
}
...
static bool cursorNearGizmo(const vec& world) {
    glm::mat4 VP = polyscope::view::getCameraPerspectiveMatrix() * polyscope::view::getCameraViewMatrix();
...
Params::Visible Params::parseVisible(const std::string& mode) {
    ...
    throw std::runtime_error("unknown visibility \"" + mode + "\" (none/panel/handle/both)");
}
...
void Params::DrawVisible(bool panel_open) {
    ...
    for (auto& [name, e] : registry) {
        ...
        if (used && (e->vis == Visible::Panel || e->vis == Visible::Both || (e->vis == Visible::Handle && !manipulable)))
    ...
    ImGui::Begin("slide parameters", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing);

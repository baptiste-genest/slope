...
public:
    ...
    static mat RotFromEulerAngles(const Eigen::Vector<float, 3>& euler_angles) {
        mat R;
        R = Eigen::AngleAxisd(euler_angles(0), vec::UnitX()) * Eigen::AngleAxisd(euler_angles(1), vec::UnitY()) * Eigen::AngleAxisd(euler_angles(2), vec::UnitZ());
        return R;
    }
};
...
class PersistentTransform {
    ...
    static bool readFile(const std::string& path, Transform& T) {
        ...
        vec s, t, axis;
        scalar angle;
        if (!(f >> s(0) >> s(1) >> s(2) >> t(0) >> t(1) >> t(2) >> axis(0) >> axis(1) >> axis(2) >> angle))
            return false;
...
public:
    ...
    std::optional<Transform> stored() const {
        ...
        Transform T;
        if (!readFile(pathOf(label), T) && !readFile(slope::Options::ProjectViewsPath + label, T))
            return std::nullopt;
